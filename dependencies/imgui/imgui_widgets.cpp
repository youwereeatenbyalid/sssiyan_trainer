...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        const float new_arrow_size = arrow_size * arrow_scale;
        if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
            RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y + (arrow_size - new_arrow_size) * 0.5f), text_col, ImGuiDir_Down, arrow_scale);
    }
...
// This is not part of ImGui but I added it because we needed it
bool ImGui::SearchBarTextInput(const char* label, const char* hint, char* buf, int buf_size, ImTextureID magnifier_icon, ImVec2 magnifier_size, const ImU32 magnifier_color, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
    ...
    // Calculate the magnifier icon values
    float magnifier_width_offset = 0.0f;
    ImRect magnifier_bb{};

	{
		float item_height = frame_bb.Max.y - frame_bb.Min.y;
		auto magnifier_bb_min_y = frame_bb.Min.y + item_height * 0.5 - magnifier_size.y * 0.5;
		auto magnifier_bb_min_x = frame_bb.Min.x + (magnifier_bb_min_y - frame_bb.Min.y);
		ImVec2 icon_min(magnifier_bb_min_x, magnifier_bb_min_y);
        magnifier_bb = { icon_min, icon_min + magnifier_size };
        magnifier_width_offset = magnifier_bb.Max.x - magnifier_bb.Min.x + 4.0;
		(*(ImRect*)&frame_bb).Min.x += magnifier_width_offset;
        inner_size.x -= magnifier_width_offset;
	}

    // Calculate the clear button values
	float clear_button_width_offset = 0.0f;
	ImRect clear_button_bb{};

	{
		float item_height = frame_bb.Max.y - frame_bb.Min.y;
		ImVec2 clear_button_size(item_height, item_height);
		auto clear_bb_max_y = frame_bb.Max.y - item_height * 0.5 + clear_button_size.y * 0.5;
		auto clear_bb_min_x = frame_bb.Max.x + 2.0f;
		ImVec2 button_min(clear_bb_min_x, clear_bb_max_y - clear_button_size.y);
		clear_button_bb = { button_min, button_min + clear_button_size };
        clear_button_width_offset = 2.0f + clear_button_bb.Max.x - clear_button_bb.Min.x;
	}
	...
	if (!is_multiline)
	{
        ImRect render_bb(ImVec4(frame_bb.Min.x - magnifier_width_offset, frame_bb.Min.y, frame_bb.Max.x + clear_button_width_offset, frame_bb.Max.y));
		RenderNavHighlight(render_bb, id);
		RenderFrame(render_bb.Min, render_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);
	}

    // Render magnifier icon
	draw_window->DrawList->AddImage(magnifier_icon, magnifier_bb.Min, magnifier_bb.Max, { 0, 0 }, { 1 , 1 }, GetColorU32(magnifier_color));
    ...
    // Render clear button
    {
		ImRect bb_interact = clear_button_bb;

		bool hovered, held;
        ImGuiID clear_button_id = GetIDWithSeed("#CLEAR", NULL, id);

        if (ButtonBehavior(clear_button_bb, clear_button_id, &hovered, &held)) {
            memset(buf, 0, buf_size);
            value_changed = true;
        }

        ImVec2 center = clear_button_bb.GetCenter();

		float cross_extent = g.FontSize * 0.5f * 0.7071f - 1.0f;
        ImU32 cross_final_color{};

        {
            ImU32 cross_col = GetColorU32(ImGuiCol_Text);
            uint32_t alpha = (cross_col & 0xFF000000) >> 24;
            alpha *= 0.4f;
            cross_col &= 0x00FFFFFF;
            cross_col |= alpha << 24;
            cross_final_color = cross_col;
        }
		...
		if (held) {
            cross_final_color = GetColorU32(ImGuiCol_Text);
		}

		center -= ImVec2(0.5f, 0.5f);
		window->DrawList->AddLine(center + ImVec2(+cross_extent, +cross_extent), center + ImVec2(-cross_extent, -cross_extent), cross_final_color, 1.0f);
		window->DrawList->AddLine(center + ImVec2(+cross_extent, -cross_extent), center + ImVec2(-cross_extent, +cross_extent), cross_final_color, 1.0f);
    }
