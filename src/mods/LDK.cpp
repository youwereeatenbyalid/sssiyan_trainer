...
std::optional<std::string> LDK::on_initialize() {
  ...
  auto capbypass_addr1 = m_patterns_cache->find_addr(base, "3B 41 30 7D 12");
  if (!capbypass_addr1) {
    return "Unable to find Cap bypass 1 pattern.";
  }
  auto capbypass_addr2 = m_patterns_cache->find_addr(base, "44 3B F0 7E 52");
  if (!capbypass_addr2) {
  ...
  auto nopfunction_1_call = m_patterns_cache->find_addr(base, "48 8B C4 55 56 57 41 54 41 56 41 57 48 8D A8");
  if (!nopfunction_1_call) {
	  return "Unable to find nop function call pattern.";
  }
  auto vergildivebomb_addr = m_patterns_cache->find_addr(base, "C7 40 10 15 00 00 00 48");
  if (!vergildivebomb_addr) {
	  return "Unable to find vergildivebomb pattern.";
  }
  auto cavforcevalid_addr = m_patterns_cache->find_addr(base, "74 04 32 C0 EB 02 B0 01 0F B6 D0");
  if (!cavforcevalid_addr) {
	  return "Unable to find cavforcevalid pattern.";
  }
  auto cavforcelightning_addr = m_patterns_cache->find_addr(base, "41 89 8D 30 0E 00 00");
  if (!cavforcelightning_addr) {
	  return "Unable to find cavforcelightning pattern.";
  }
  auto cavcoordinatechange_addr = m_patterns_cache->find_addr(base, "49 8B 4D 10 F3 0F 11 45 90");
  if (!cavcoordinatechange_addr) {
	  return "Unable to find cavcoordinatechange pattern.";
  }
  auto hitvfxskip_addr = m_patterns_cache->find_addr(base, "75 F3 48 85 ED");
  if (!hitvfxskip_addr) {
    return "Unable to find hitvfxskip_addr pattern.";
  }

  auto waittime_addr = m_patterns_cache->find_addr(base, "49 63 40 24 48 89 5C 24 50"); //DevilMayCry5.exe+11C5E9C 
  if (!waittime_addr) {
    return "Unable to find waittime_addr pattern.";
  }

  auto dontdrawhitlines_addr = m_patterns_cache->find_addr(base, "76 80 78 51 00 74 04"); //DevilMayCry5.exe+916E9D
