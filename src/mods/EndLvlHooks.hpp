...
public:
    ...
    //Call all shit that should be called when end of level/bp/secret nission happens here:
    static void end_actions_asm()
    {
        WaveEditorMod::EnemyWaveEditor::restore_list_data_asm();
        if(EnemySwapper::cheaton)
            EnemySwapper::clear_swap_data_asm();
        if(DMC3JCE::cheaton)
            DMC3JCE::jceController->stop_jce();
    }
