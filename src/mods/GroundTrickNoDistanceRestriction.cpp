...
std::optional<std::string> GroundTrickNoDistanceRestriction::on_initialize()
{
  ...
  set_up_hotkey();
	...
	auto distanceAddr0 = m_patterns_cache->find_addr(base, "F3 0F 10 41 14 48 8B 4F"); //DevilMayCry5.exe+10BFE80
	if (!distanceAddr0) {
		return "Unanable to find GroundTrickNoDistanceRestriction.distanceAddr0 pattern.";
	}

	auto distanceAddr1 = m_patterns_cache->find_addr(base, "74 C0 F3 0F 10 48 14"); //DevilMayCry5.exe+10C18E8
	if (!distanceAddr1) {
