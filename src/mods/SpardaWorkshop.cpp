...
std::optional<std::string> SpardaWorkshop::on_initialize() {
  ...
  auto sceneplacer_addr = m_patterns_cache->find_addr(base, "8B 42 08 89 41 38 80");
  if (!sceneplacer_addr) {
    return "Unable to find sceneplacer pattern.";
  }
  auto enemyplacer_addr = m_patterns_cache->find_addr(base, "01 00 00 F3 0F 10 86 80 00 00 00 F3");
  if (!enemyplacer_addr) {
