...
glm::mat4x4 WorldVisualizer::viewProjectionMatrix;

void WorldVisualizer::SetViewProjectionMatrix(const glm::mat4x4& matrix) {
...
bool WorldVisualizer::WorldToScreen(const glm::vec3& worldPos, ImVec2& screenPos) {
    ...
    ImVec2 displaySize = ImGui::GetIO().DisplaySize;
    screenPos.x = (ndc.x + 1.0f) * 0.5f * displaySize.x;
    screenPos.y = (1.0f - ndc.y) * 0.5f * displaySize.y;
...
void WorldVisualizer::DrawWorldLine(const glm::vec3& start, const glm::vec3& end, ImU32 color, float thickness) {
...
void WorldVisualizer::DrawWorldTriangle(const glm::vec3& p0, const glm::vec3& p1, const glm::vec3& p2, ImU32 color, float thickness) {
    DrawWorldLine(p0, p1, color, thickness);
    DrawWorldLine(p1, p2, color, thickness);
    DrawWorldLine(p2, p0, color, thickness);
}

void WorldVisualizer::DrawWorldFilledTriangle(const glm::vec3& p0, const glm::vec3& p1, const glm::vec3& p2, ImU32 color) {
...
glm::mat3x3 WorldVisualizer::CreateRotationMatrix(float pitchRad, float yawRad, float rollRad) {
...
glm::vec3 WorldVisualizer::RotatePoint(const glm::vec3& point, const glm::vec3& center, const glm::mat3x3& rotation) {
    glm::vec3 local = point - center;
    glm::vec3 rotated;
    rotated.x = local.x * rotation[0][0] + local.y * rotation[1][0] + local.z * rotation[2][0];
    rotated.y = local.x * rotation[0][1] + local.y * rotation[1][1] + local.z * rotation[2][1];
    rotated.z = local.x * rotation[0][2] + local.y * rotation[1][2] + local.z * rotation[2][2];
    return rotated + center;
}

void WorldVisualizer::DrawWorldSphere(const glm::vec3& center, float radius, ImU32 color, int segments, float thickness, const glm::mat3x3* rot) {
    glm::mat3x3 rotation = rot ? *rot : glm::mat3(1.0f);
    ...
    for (int i = 0; i < segments; ++i) {
        ...
        glm::vec3 p2 = { center.x + radius * cosf(a2), center.y + radius * sinf(a2), center.z };
    ...
    for (int i = 0; i < segments; ++i) {
        ...
        glm::vec3 p1 = { center.x + radius * cosf(a1), center.y, center.z + radius * sinf(a1) };
        glm::vec3 p2 = { center.x + radius * cosf(a2), center.y, center.z + radius * sinf(a2) };
    ...
    for (int i = 0; i < segments; ++i) {
        ...
        glm::vec3 p1 = { center.x, center.y + radius * cosf(a1), center.z + radius * sinf(a1) };
        glm::vec3 p2 = { center.x, center.y + radius * cosf(a2), center.z + radius * sinf(a2) };
...
void WorldVisualizer::DrawWorldCapsule( const glm::vec3& start, const glm::vec3& end, float radius, ImU32 color, int segments, float thickness, const glm::mat3x3* rot) {
    glm::mat3x3 rotation = rot ? *rot : glm::mat3(1.0f);
    ...
    glm::vec3 axis = dir / length; // normalized axis
    ...
    for (int i = 0; i < segments; ++i) {
        ...
        glm::vec3 offset1 = { radius * cosf(angle1), 0.0f, radius * sinf(angle1) };
        glm::vec3 offset2 = { radius * cosf(angle2), 0.0f, radius * sinf(angle2) };

        // Rotate offsets if a rotation is provided
        if (rot) {
            offset1 = WorldVisualizer::RotatePoint(offset1, glm::vec3{ 0,0,0 }, rotation);
            offset2 = WorldVisualizer::RotatePoint(offset2, glm::vec3{ 0,0,0 }, rotation);
        }

        // Connect start and end circles
        glm::vec3 p1_start = start + offset1;
        glm::vec3 p2_start = start + offset2;
        glm::vec3 p1_end = end + offset1;
        glm::vec3 p2_end = end + offset2;
...
void WorldVisualizer::DrawWorldFilledSphere(const glm::vec3& center, float radius, ImU32 color) {
    ImVec2 screenPos;
...
void WorldVisualizer::DrawWorldBox(const glm::vec3& min, const glm::vec3& max, ImU32 color, float thickness) {
    glm::vec3 corners[8];
    corners[0] = glm::vec3(min.x, min.y, min.z); // 0
    corners[1] = glm::vec3(max.x, min.y, min.z); // 1
    corners[2] = glm::vec3(max.x, max.y, min.z); // 2
    corners[3] = glm::vec3(min.x, max.y, min.z); // 3
    corners[4] = glm::vec3(min.x, min.y, max.z); // 4
    corners[5] = glm::vec3(max.x, min.y, max.z); // 5
    corners[6] = glm::vec3(max.x, max.y, max.z); // 6
    corners[7] = glm::vec3(min.x, max.y, max.z); // 7
...
void WorldVisualizer::DrawWorldPoint(const glm::vec3& pos, float size, ImU32 color) {
    ImVec2 screenPos;
...
void WorldVisualizer::DrawWorldText(const glm::vec3& pos, const char* text, ImU32 color) {
    ImVec2 screenPos;
    if (WorldToScreen(pos, screenPos)) {
...
void WorldVisualizer::DrawWorldArrow(const glm::vec3& start, const glm::vec3& end, ImU32 color, float thickness, float arrowSize) {
    ImVec2 screenStart, screenEnd;
    if (WorldToScreen(start, screenStart) && WorldToScreen(end, screenEnd)) {
        // Draw main line
        GetDrawList()->AddLine(screenStart, screenEnd, color, thickness);
...
void WorldVisualizer::DrawWorldPolyline(const std::vector<glm::vec3>& points, ImU32 color, float thickness, bool closed) {
    if (points.size() < 2)
        return;
...
void WorldVisualizer::DrawWorldGrid(const glm::vec3& center, float size, int divisions, ImU32 color, float thickness) {
